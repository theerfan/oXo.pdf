...
router.post('/upload', koaBody({ multipart: true }), async (ctx: Context) => {
    ...
    if (file) {
        ...
        const filePath = `/home/oxo.bun/public/uploads/${file.originalFilename}`;
...
async function displayPDF(url: string): Promise<void> {
    ...
    // pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.6.347/pdf.worker.entry.js';
